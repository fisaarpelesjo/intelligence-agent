"""SlackDeliveryAdapter — T085 (`D-23` undeclared; FR-053, FR-061, FR-071; SC-033).

Implements `delivery.ports.DeliveryPort` and nothing else.

**Unconstructible today.** `D-23` is undeclared, so a Slack application, its scopes, a bot
identity and a signing secret do not exist, and none is created here. :func:`build_slack_delivery`
refuses instead of returning a half-configured adapter — a client that existed but could not send
would turn a governed lock into a runtime surprise.

Slack addresses a conversation by channel or member id. Those are provider identifiers for a
person or a room, so the same rule applies: the core holds a reference and the resolution lives
here, against `D-23`'s installation — which does not exist.
...
__all__ = ["CHANNEL", "CREDENTIAL_RECORD", "SlackDeliveryAdapter", "build_slack_delivery"]
...
class SlackDeliveryAdapter:
    """Sends a rendered presentation to slack. Holds a reference, never material."""
    ...
    def send(
        ...
        With `D-23` undeclared the resolver yields no material, so this reports ``WITHHELD`` with no
        ...
        # Reached only once `D-23` is declared and material resolves. The provider call itself
        # belongs to the transport surface `D-30` owns, so there is deliberately no client here to
        # call: an adapter that constructed one would be creating the very endpoint `D-23` governs.
        raise ChannelViolation(
            ChannelReasonCode.CHANNEL_NOT_CONFIGURED,
...
def build_slack_delivery(ref: SecretRef, resolver: SecretResolver) -> SlackDeliveryAdapter:
    """The only constructor, and it refuses while `D-23` is undeclared.
    ...
    enable this channel while `D-23` is open (`FR-097`).
    """
    ...
    return SlackDeliveryAdapter(ref, resolver)
