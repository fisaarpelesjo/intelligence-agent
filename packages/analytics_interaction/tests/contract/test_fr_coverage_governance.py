"""FR coverage: governed content, clarification, audit, observability and readiness — T172.

    Evidence: each FR in scope resolves to a module **and** a passing executable
    validation. — `tasks.md` T172
...
An FR belongs to this domain when a `src/clarification/`, `src/governance/`,
`src/compliance/`, `src/audit/`, `src/telemetry/` or `src/cli/` module cites it,
when one of the four governance contracts does, or when **only** a cross-cutting
module or an ADR cites it — this is the fallback domain, because every such
requirement so far is a prohibition, a reason-code rule or a readiness rule.
...
def test_the_domain_has_a_non_trivial_scope() -> None:
    ...
    60 is a floor rather than an exact count: the scope is derived, so a legitimate
    ...
    """
    assert len(IN_SCOPE) >= 60, IN_SCOPE
