"""FR coverage: comparison, answer assembly and caveats — T171.

    Evidence: each FR in scope resolves to a module **and** a passing executable
    validation. — `tasks.md` T171
...
An FR belongs to this domain when a `src/comparison/` or `src/answer/` module
cites it, or when `comparison-contract.md` or `answer-contract.md` does.
...
def test_the_domain_has_a_non_trivial_scope() -> None:
    ...
    25 is a floor rather than an exact count: the scope is derived, so a legitimate
    ...
    """
    assert len(IN_SCOPE) >= 25, IN_SCOPE
